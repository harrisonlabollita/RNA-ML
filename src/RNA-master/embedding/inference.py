...
def load_args():
    ...
    parser.add_argument(
        '--embedding', type=int, default=0, help='Use model with Embedding layers')
    parser.add_argument(
        '--hidden_dim',
        type=int,
        default=64,
...
if __name__ == '__main__':
    ...
    RNA = RNA2STRUCTURE(
        MAX_SEQ_LENGTH_ENC, MAX_SEQ_LENGTH_DEC, trainable=False)
    ...
    RNA.load_weights(
        WEIGHTS_FPATH, WEIGHTS_ENCODER, WEIGHTS_DECODER, attention=ATTENTION)
    ...
    ###############################################################
    """ 
        Load training / testing data with batch_size = 1 for inference. 
    """
    ...
    for (x, y, z) in test_gen:
        ...
        if USE_EMBEDDING:
            pred_seq, attention_weights = RNA.InferFromEmbedding(xs)
            x_decode = xs[0]
        else:
            pred_seq, attention_weights = RNA.InferFromHot(xs)
