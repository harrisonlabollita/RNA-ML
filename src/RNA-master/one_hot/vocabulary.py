...
class RNACoder:
    ...
    def one_hot_encode(self, sequence):
        encoding = list()
        for i in range(self.MAX_SEQ_LENGTH_DEC):
            try:
                value = sequence[i]
            except:
                value = '-'
    ...
    def get_pair(self, sequence_in, sequence_out):
        ...
        X = self.one_hot_encode(sequence_in)
        y = self.one_hot_encode(sequence_out)
...
def load_generators(DATA_FILEPATH, MAX_SEQ_LENGTH_ENC,
    ...
    print("There are {} sequences with length <= {}".format(
        len(structures), MAX_SEQ_LENGTH_ENC))

    total_chars = sum([len(x[0]) + 2 for x in structures])
