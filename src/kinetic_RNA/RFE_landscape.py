...
class RNALandscape:
    ...
    def __init__(self, sequences, DNA = [False,False], sequenceFrequencies = [1,1], b = 0.8/0.33, T = 310.15,
                  ...
                  considerC3andC4 = False,  corruptFESeed = False, unboundButCouldBindPenalties = [0,0]):
...
#        We cut this in half by only considering RNA(DNA) for systems that only have RNA(DNA)
        ...
        self.indexSort = np.flip(np.argsort(probabilities),0) #flip so that probabilities are decreasing
...
#        and its FE will be given by sortedFEs[n]
        ...
        if storeGraphs:
            ...
            self.indexSortedGraphProbs = np.flip(np.argsort(graphProbs),0)
